...
public class ChestnutFoliagePlacer extends FoliagePlacer {
    ...
    @Override
    protected void createFoliage(LevelSimulatedReader level, FoliageSetter consumer, RandomSource rand, TreeConfiguration config, int p_161426_, FoliageAttachment foliage, int p_161428_, int p_161429_, int p_161430_) {
        for (int x = -1; x <= 1; x++)
            for (int y = -1; y <= 1; y++)
                for (int z = -1; z <= 1; z++)
